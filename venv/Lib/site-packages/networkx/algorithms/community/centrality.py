...
def girvan_newman(G, most_valuable_edge=None):
    ...
    if most_valuable_edge is None:
        def most_valuable_edge(G):
            ...
            myedge = max(betweenness, key=betweenness.get)
            try:
                myedge_list = []
                for k, v in betweenness.items():
                    if v == betweenness[myedge]:
                        myedge_list.append(k)
                myweight = ''
                mymin_edge = (0, 0)
                for item in myedge_list:
                    u = item[0]
                    v = item[1]
                    if myweight == '':
                        myweight = G[u][v]['weight']
                        mymin_edge = (u, v)
                    else:
                        if (myweight > G[u][v]['weight']):
                            myweight = G[u][v]['weight']
                            mymin_edge = (u, v)
            except KeyError:
                return myedge
